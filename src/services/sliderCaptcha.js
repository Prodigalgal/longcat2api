...
/**
 * Detect classic slider UI (not connect-dots sudoku).
 */
export async function detectSlider(page) {
  const handleSels = [
    ...
    '[class*="slide-btn"]',
    '[class*="sliderMove"]',
    '.yoda-slider-wrapper [class*="btn"]',
    '#yodaBox [class*="slider"] [class*="btn"]',
    '.yoda-modal-content [class*="slider"] > div',
  ];
  for (const sel of handleSels) {
    const loc = page.locator(sel).first();
    if (await loc.isVisible().catch(() => false)) {
      const box = await loc.boundingBox().catch(() => null);
      // handle is usually small square/circle
      if (box && box.width < 80 && box.height < 80) {
        return { kind: 'slider', handle: loc, selector: sel };
      }
      if (box && box.width <= box.height * 2.5) {
        return { kind: 'slider', handle: loc, selector: sel };
      }
    }
  }

  // Heuristic: any small movable knob near bottom of yoda modal
  const candidates = await page.evaluate(() => {
    const root = document.querySelector('#yodaVerify, .yoda-verify-container, .yoda-modal-content') || document.body;
    const out = [];
    for (const el of root.querySelectorAll('div, span, button')) {
      const r = el.getBoundingClientRect();
      if (r.width < 20 || r.width > 70 || r.height < 20 || r.height > 70) continue;
      const cls = (el.className || '').toString().toLowerCase();
      if (/slider|slide|handler|btn|move|drag|block/.test(cls) || el.getAttribute('role') === 'button') {
        out.push({
          cls: cls.slice(0, 80),
          w: r.width,
          h: r.height,
  ...
  if (candidates.length) {
    // pick bottom-most small element
    candidates.sort((a, b) => b.y - a.y);
    const c = candidates[0];
    const handle = page.locator(`[class*="${(c.cls.split(/\s+/)[0] || 'slider').replace(/[^\w-]/g, '')}"]`).first();
    // better: use position via mouse without handle locator
    return {
      kind: 'slider',
      handle: null,
      selector: 'heuristic',
      start: { x: c.x + c.w / 2, y: c.y + c.h / 2 },
    };
  }

  const body = await page.locator('body').innerText().catch(() => '');
  if (/滑块|向右滑动|拖动滑块|slide to verify|drag the slider/i.test(body)) {
    return { kind: 'slider_text', handle: null, selector: 'text-only' };
  }
  return null;
}
